...
def initialize_bem_scoring_function(vocab_path=_VOCAB_PATH,
  ...
  def preprocess_example(example):
    ...
    # [CLS] candidate [SEP] reference [SEP] question [SEP]
...
def encyclopedic_vqa_evaluation_function(example,
  ...
  if matches_exactly:
    return 1.0
  return bem_scoring_function(example, threshold_score=False)

def batch_encyclopedic_vqa_evaluation_function(examples,
                                         bem_scoring_function):
  batch_scores = []
  for example in tqdm(examples):
    ...
    batch_scores.append(bem_scoring_function(example, threshold_score=False))
  return batch_scores
...
def initialize_encyclopedic_vqa_evaluation_function(
  ...
  return functools.partial(
      batch_encyclopedic_vqa_evaluation_function,
...
def evaluate_example(
  ...
  scores = []
  examples = []
  for question, reference, candidate in zip(question_list, reference_list, candidate_list):
    example = {
        'question': question,
        'reference': reference,
        'candidate': candidate,
  ...
  scores = scoring_function(examples)
  return scores